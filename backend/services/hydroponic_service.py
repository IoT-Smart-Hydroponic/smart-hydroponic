...
from schemas.hydroponic import HydroponicIn, HydroponicOut, MetaData, ResponseList
...
class HydroponicService:
    ...
    async def get_all_data(
        ...
        records = await self.repo.get_all_hydroponic_data(
            page, limit, start_date, end_date
        )
        return ResponseList(
            meta=MetaData(
                total_rows=records.total, limit=limit, offset=(page - 1) * limit
            ),
            data=records.data,
        )
    ...
    async def get_specific_data(
        ...
        records = await self.repo.get_specific_hydroponic_data(
