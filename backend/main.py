...
async def lifespan(app: FastAPI):
    ...
    root.add_resource(["coap","hydroponics", "sensor"], HydroponicCoAPResource(
        role="sensor",
        aggregator=aggregator,
        manager=manager,
    ))
    root.add_resource(["coap","hydroponics", "environment"], HydroponicCoAPResource(
        role="environment",
        aggregator=aggregator,
        manager=manager,
    ))
    root.add_resource(["coap","hydroponics", "actuator"], HydroponicCoAPResource(
        role="actuator",
        aggregator=aggregator,
        manager=manager,
    ))

    coap_context = await aiocoap.Context.create_server_context(root, bind=('localhost', 5683))
